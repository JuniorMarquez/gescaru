(function () {
    ...
    function PagoController(logger,
        ...
        if (MyService.data.x == "si"){
            monto = parseInt(MyService.data.extra5) ;
            vm.pago.monto = monto.toFixed(2);
        }
         if (MyService.data.x == "no")
            {
            vm.pago.monto = 0;
            vm.dato = MyService.data.dato;
            }
        if (MyService.data.x == "nop")
            {
             vm.val = MyService.data.val;
            vm.dato = MyService.data.dato;
            var monto = vm.dato/vm.val;
            vm.pago.monto = parseFloat(monto.toFixed(2));
            }
            if (MyService.data.x == "nos")
            {
             vm.val = MyService.data.val;
            vm.dato = MyService.data.dato;
            var monto = vm.dato;
...
vm.create3= function() {
            // Create new Pago object
            vm.pago.descripcion = "pago de cuota";
            var pago = new Pago(vm.pago);
        ...
        vm.create4= function() {
            // Create new Pago object
            vm.pago.descripcion = "pago de monto restante";
            var pago = new Pago(vm.pago);
